...
class SnmpDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, config_entry):
        ...
        # SNMP client is initialized asynchronously in async_config_entry_first_refresh()
        # to avoid blocking the event loop during SnmpEngine() instantiation.
        self.client = None
    ...
    async def async_set_switch_state(self, key: str, state: bool, port: str = None):
        ...
        # Guard: client not yet initialized (first poll hasn't run)
        if self.client is None:
            _LOGGER.warning("SNMP client not yet initialized, skipping set_switch_state")
            return False
    ...
    async def async_set_text_value(self, key: str, value: str, port: str = None):
        ...
        # Guard: client not yet initialized (first poll hasn't run)
        if self.client is None:
            _LOGGER.warning("SNMP client not yet initialized, skipping set_text_value")
            return False
    ...
    async def _async_update_data(self):
        ...
        # Lazily initialize the SNMP client on first poll (avoids blocking event loop in __init__)
        if self.client is None:
            self.client = await SnmpClient.create(
                self.config_entry.data[CONF_DEVICE_IP],
                self._create_client_credentials()
            )
            _LOGGER.info("SNMP client initialized asynchronously")
        ...
        async with self._lock:
            ...
            try:
                ...
                if current_time - self._last_mac_update >= mac_interval:
                    ...
                    excluded_ports = set(self.config_entry.options.get("mac_excluded_ports", []))
                        ...
                        if mac_table_oid and mac_port_oid:
                            ...
                            if macs and ports:
                                ...
                                for suffix, mac_val in mac_suffix_map.items():
                                    ...
                                    if port_str in excluded_ports:
                                        continue  # skip ports the user has excluded
